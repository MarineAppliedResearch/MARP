...
Description:
This script automatically annotates images in a dataset folder with their corresponding YOLO-format annotations.
It recursively searches through 'train' and 'val' subdirectories in the 'dataset/images' folder.
As each image is processed, the script reads the associated label file from the corresponding 'train' or 'val' subdirectory in 'dataset/labels',
draws the bounding box annotations on the image, and saves the annotated image to a designated output folder.

Features:
- Recursively annotates images from the 'train' and 'val' subdirectories of the specified dataset folder.
- Reads YOLO-format label files and overlays bounding boxes on images.
- Saves all annotated images to a designated output folder.
...
output_folder = 'converted_images'        # Folder to save annotated images
...
# Main loop to process images with annotations and save them
...
print("\nAll images have been processed and annotated.")
